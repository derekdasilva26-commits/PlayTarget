...
def create_price(price: PriceCreate, db: Session = Depends(get_db), current_user: object = Depends(get_current_user)):
    """Criar um novo preço"""
...
def update_price(price_id: int, price_update: PriceUpdate, db: Session = Depends(get_db), current_user: object = Depends(get_current_user)):
    """Atualizar o preço"""
...
def delete_price(price_id: int, db: Session = Depends(get_db), current_user: object = Depends(get_current_user)):
    """Deletar um preço"""
    db_price = db.query(Price).filter(Price.id == price_id).first()
    if not db_price:
        raise HTTPException(status_code=404, detail="Preço não encontrado")
